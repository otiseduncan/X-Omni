...
SCRAPEX_STATUS_SCHEMA: dict[str, Any] = {
    "description": (
        ...
        "state. Do not use this to answer whether Calibration IQ already has ADAS Map "
        "documents or which ROs are missing them; that is a Calibration IQ read. This is "
        "not ALLDATA status, does not use ALLDATA credentials, and does not retrieve OEM "
        "service-information procedures. It opens nothing."
    ),
    "parameters": {
        "type": "object",
        "properties": {},
...
SCRAPEX_READ_SCHEMA: dict[str, Any] = {
    "description": (
        "Read ScrapeX ADAS Map acquisition batches, exact-RO acquisition evidence, "
        "exceptions, or a non-mutating acquisition-queue preview. This is not the source "
        "for current Calibration IQ ADAS Map presence, missing-map counts, phase coverage, "
        "or current RO state; use Calibration IQ for those questions. This tool is never "
        "an ALLDATA/SI procedure source. For a create result, batch_id is exactly "
        "result.data.id, never evidence_id. Existing ScrapeX acquisition evidence begins "
        "with list_batches when no id is known; new ADAS Map acquisition uses "
        "scrapex_adas_map instead."
    ),
    "parameters": {
        "type": "object",
        "oneOf": [
            ...
            },
            {
                ...
                "properties": {
                    "action": {
                        ...
                        "const": "acquire_exact",
                        "description": (
                            "Acquire one exact RO now, save its ADAS Map PDF, attach the "
                            "document to the matching Calibration IQ RO, and return the "
                            "verified terminal result. Use this for a single-RO request."
                        ),
                    },
                    "ro_number": {"type": "string", "maxLength": MAX_RO_CHARS},
                    "source_scope": {
...
def _verified_local_adas_map(
    ...
    root = getattr(settings, "adas_si_root", None)
    if root is None:
        return {
            "verified": False,
            "relative_path": f"ADAS Map/{ro_number}/{ro_number} ADAS Map.pdf",
            "message": "ADAS SI root is not configured.",
        }
    try:
        path = adas_storage.adas_map_pdf_path(Path(root), ro_number)
        if not path.is_file() or path.stat().st_size < 256:
            raise OSError("canonical ADAS Map PDF is missing or empty")
        with path.open("rb") as handle:
            if handle.read(5) != b"%PDF-":
                raise OSError("canonical ADAS Map file is not a PDF")
        relative = path.resolve().relative_to(Path(root).resolve()).as_posix()
        pages_total = adas.page_count(path)
        return {
            "verified": True,
            "path": str(path),
...
def _ciq_attachment_from_result(
    result: dict[str, Any],
    *,
    ro_id: str,
    source_uri: str,
    ...
    final = (
        (result.get("final_snapshots") or {}).get(ro_id)
        if isinstance(result.get("final_snapshots"), dict)
        else None
    )
    snapshot = final.get("snapshot") if isinstance(final, dict) else None
    documents = (
        calibration_iq._existing_research_documents(snapshot)  # noqa: SLF001
    ...
    if len(matches) != 1:
        return {
            ...
            "message": (
                "Calibration IQ did not return one exact managed ADAS Map "
                "document after the attachment operation."
            ),
        }
    document = matches[0]
    document_id = str(
    ...
    if not download_url and document_id:
        download_url = str(
            calibration_iq._document_proxy_url(document_id) or ""  # noqa: SLF001
        )
    return {
        "status": "verified",
        "attached": True,
...
async def adas_map_with_ciq_attachment(
    settings: Any,
    adas: Any,
    args: dict[str, Any],
) -> dict[str, Any]:
    ...
    local = _verified_local_adas_map(settings, adas, ro_number)
    result = {**result, "local_report": local}
    if local.get("verified") is not True:
        return {
            **result,
    ...
    ro_read = await calibration_iq.get_repair_order(
        settings, {"ro_number": ro_number}
    )
