...
TRUTH_REVIEW_INSTRUCTION = """Internal truth review; this is not a new user request. Judge only whether the assistant's last message contains a material factual or execution claim that contradicts or exceeds the authoritative tool results in this conversation. A short answer is valid when the evidence supports it; do not require receipts, ids, versions, counts, timestamps, or audit detail unless omitting them makes the message materially false. Brevity is never a defect and extra detail is never required. Never infer success the evidence does not state: authentication required, conflict, partial, indeterminate, may-have-executed, failed, and unverified are not success. Restating a failure reason the evidence itself carries -- an error code, message, or status it contains -- is supported, not an overreach. Do not rewrite the answer and do not choose tools. When correction is needed keep correction_guidance to one short sentence naming only the claim to drop or fix; never ask for receipts, ids, versions, timestamps, or a fuller breakdown. Reply with exactly one JSON object and nothing else:
