...
For "prepare for the week" or a phase/RO-scoped SI-readiness audit, use `calibration_iq_work_prep` (`mode: week_readiness` for the default phase 5-8 sweep, `phase_coverage` for one explicit phase, `ro_requirements` for one RO, `phase_list` to just list a phase). Its result is a real evidence card — a subsequent question in the same conversation about which ROs need SI, are missing an ADAS Map, or are otherwise exceptions should be answered from that stored result's `repair_orders`/`needs_si_count`/`exception_count` fields, not by falling back to a generic `calibration_iq_summary`/`calibration_iq_read` call with invented shop or phase filters — that result has no relationship to which vehicles need SI collected. If the stored result is missing, stale, or was truncated below what the question needs, say so and offer to re-run `week_readiness` rather than guessing. Once Otis is working the list vehicle-by-vehicle in ALLDATA, use `mode: queue_next` ("next") to resolve the currently-selected vehicle against the remaining queue instead of asking him to repeat the RO or vehicle every turn.
