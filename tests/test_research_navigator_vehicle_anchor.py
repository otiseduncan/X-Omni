...
def _module():
    ...
    module._run_one_task = run_one_task
    module.calls = calls
    return module
...
async def test_valid_vin_forces_one_reselection_then_uses_real_signal():
    ...
    first, second = await module._run_one_task(target={"vin": VIN})
...
async def test_each_new_task_gets_a_fresh_vehicle_anchor():
    ...
    assert await module._run_one_task(target={"vin": VIN}) == (False, True)
    assert await module._run_one_task(target={"vin": VIN}) == (False, True)
...
async def test_task_without_exact_vin_keeps_existing_behavior():
    ...
    first, second = await module._run_one_task(target={"year": 2025, "make": "Kia", "model": "K4"})
