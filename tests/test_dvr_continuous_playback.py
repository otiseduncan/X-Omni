...
def test_same_recorder_session_ignores_mtime_end_jitter() -> None:
    rows = [
        _row(10, "2026-08-30T06:40:00Z", "2026-08-30T06:44:58Z", filename=_session_name(0)),
        _row(11, "2026-08-30T06:45:00Z", "2026-08-30T06:49:58Z", filename=_session_name(1)),
        _row(12, "2026-08-30T06:50:00Z", "2026-08-30T06:55:01Z", filename=_session_name(2)),
    ]
...
def test_missing_ordinal_is_a_real_break_even_if_wall_clock_is_close() -> None:
    first = _row(20, "2026-08-30T06:40:00Z", "2026-08-30T06:45:00Z", filename=_session_name(0))
    skipped = _row(22, "2026-08-30T06:50:00Z", "2026-08-30T06:55:00Z", filename=_session_name(2))
...
def test_ffconcat_uses_media_timing_not_sqlite_close_time(tmp_path: Path) -> None:
    ...
    a = tmp_path / "a.mkv"
    b = tmp_path / "b.mkv"
    parts = [PlaybackPart(first, a, 120.0), PlaybackPart(second, b, 0.0)]
...
def test_historical_playback_cannot_block_the_dvr_service_indefinitely() -> None:
    # File-backed playback should emit bytes continuously. Keep this comfortably
    # below a human-visible page freeze; a wedged segment is isolated instead of
    # being allowed to hold the whole DVR console hostage.
    assert 1.0 <= STREAM_NO_PROGRESS_SECONDS <= 15.0
